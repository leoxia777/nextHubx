...
/**
 * nextHubx 导航裁剪(最终 spec):
 * - `defaultNavItems`:默认导航 = 定制后的 CVR 原生页 **Home / Logs / Test / Settings**。
 *   连接逻辑取消(TUN 锁定常开),激活逻辑并入 Home 的 Account 卡片;
 *   `nexthubx-connect/activate/account` 三页移出默认导航(路由保留可达,激活页兼作「重新激活」入口)。
 * - `advancedNavItems`:原生排障页(proxies/profiles/connections/rules/unlock),默认从侧边栏隐藏,
 *   仅当「高级/调试入口」开启时才追加到导航中。
 * - `navItems`:全量项,**仅用于注册路由**——所有页路由始终可达(直接输 URL / 程序跳转),
 *   只是默认不在侧边栏出现。
 * - 根路径 `/` 重定向到 Home(`/home`)。
 *
...
/**
 * 仅注册路由、不进侧边栏的页:废弃的 nextHubx 定制页(连接 / 激活 / 账号)。
 * 激活页保留可达,供 Home 的 Account 卡片「重新激活」跳转。
 */
...
/** 全量导航项(默认页 + 高级页),仅用于注册路由 / 侧边栏可选项。 */
export const navItems = [...defaultNavItems, ...advancedNavItems]

/** 注册到路由的全部页(含默认 + 高级 + 隐藏路由)。 */
const routableItems = [...navItems, ...hiddenRouteItems]
