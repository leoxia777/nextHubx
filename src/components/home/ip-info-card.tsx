      ...
      mainElement = (
        <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
          <Box sx={{ flex: 1, overflow: 'hidden' }}>
            ...
            {/* 多源交叉校验:并发探测多个 IP 源,列出各源结果。某源限频/失败标明并提示自行去查。
                多源 IP 一致即出口 IP 可信;自有源 ip.nexthubx.io 置首作主结果。 */}
            {ipInfo?.probes && ipInfo.probes.length > 0 && (
              <Box
                sx={{ mt: 1, pt: 0.5, borderTop: 1, borderColor: 'divider' }}
              >
                <Typography
                  variant="caption"
                  color="text.secondary"
                ...
                >
                  {t('home.components.ipInfo.sources.title')}
                </Typography>
                <Box sx={{ maxHeight: 116, overflow: 'auto' }}>
                  {ipInfo.probes.map((p) => (
                    <Box
                      key={p.source}
                      sx={{
                    ...
                    >
                      ...
                      {p.status === 'ok' ? (
                        ...
                        >
                          {p.status === 'ratelimited'
                            ? t('home.components.ipInfo.sources.ratelimited')
                            : t('home.components.ipInfo.sources.failed')}
                          {' · '}
                          {t('home.components.ipInfo.sources.checkManually')}
                        </Typography>
                      )}
                    </Box>
                  ))}
